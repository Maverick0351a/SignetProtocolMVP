...
def _client(tmp_path, monkeypatch, secret: bytes):
    ...
    monkeypatch.setenv("SIGNET_SIGNING_KEY_PATH", str(tmp_path / "keys/ed25519_private.key"))
    monkeypatch.setenv("SIGNET_SIGNING_PUBKEY_PATH", str(tmp_path / "keys/ed25519_public.key"))
    monkeypatch.setenv("SIGNET_INGRESS_HMAC_PATH", str(tmp_path / "keys/ingress_hmac.json"))
...
def _signed(prepared_secret: bytes, payload):
    ...
    signer = HTTPMessageSigner(signature_algorithm=algorithms.HMAC_SHA256, key_resolver=_StaticResolver("k1", prepared_secret))
    signer.sign(prepared, key_id="k1", covered_component_ids=("@method", "@path", "content-digest"))
...
def test_policy_blocked_payload_returns_403(tmp_path, monkeypatch):
    ...
    resp = client.post("/vex/exchange", data=prepared.body, headers=dict(prepared.headers))
