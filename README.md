...
[![CodeQL](https://github.com/Maverick0351a/SignetProtocolMVP/actions/workflows/codeql.yml/badge.svg)](https://github.com/Maverick0351a/SignetProtocolMVP/actions/workflows/codeql.yml)
...
[![codecov](https://codecov.io/gh/Maverick0351a/SignetProtocolMVP/branch/main/graph/badge.svg)](https://codecov.io/gh/Maverick0351a/SignetProtocolMVP)
[![License: Apache-2.0](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)
![Python](https://img.shields.io/badge/python-3.11%20|%203.12-blue)
[![PyPI](https://img.shields.io/pypi/v/signet-sdk)](https://pypi.org/project/signet-sdk/)
...
Cross-language Verification (Node)

An independent Node.js verifier proves receipts generated by the Python service are implementation‑agnostic.

Why it matters:
- Detects accidental Python‑specific canonicalization quirks (we use RFC 8785 JSON Canonicalization Scheme in both impls).
- Confirms Ed25519 signatures produced from raw 32‑byte keys verify after Node key material adaptation.
- Strengthens supply‑chain & audit story (multi‑implementation verification reduces single‑runtime risk).

Script: scripts/verify-receipt-node.mjs

Prereqs: Node 18+. (No external deps; uses built‑in crypto.)

Typical flow (after running an exchange request to produce a receipt under storage/receipts/YYYY-MM-DD/...):

```
# (Optional) create a fresh receipt via API
curl -X POST http://127.0.0.1:8000/exchange \
	-H "Content-Type: application/json" \
	-d '{"content":"hello crosslang"}' > /dev/null

# Pick the newest receipt file
$latest=(Get-ChildItem -File -Recurse storage/receipts | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName

# Verify with Node (PowerShell shown)
node scripts/verify-receipt-node.mjs "$latest" keys/ed25519_public.key
```

Expected output:

```
OK  signature verifies (JCS canonicalization + Ed25519)
```

Failure modes:
- Canonicalization mismatch (fields reordered / spacing) -> digest mismatch -> signature fail.
- Wrong / rotated public key -> crypto.verify returns false.
- Receipt tampering (any byte change) -> computed payload hash differs -> signature fail.

CI Integration: The main test workflow runs this script against a generated sample receipt; any failure breaks the build.

Limitations / Next Steps:
- Current Node script validates signature & canonical form; Merkle inclusion & chain prev_receipt_hash_b64 validation stay in Python for now.
- Plan: add optional --prev flag to independently recompute prior hash linkage, and a small inclusion proof verifier once proofs.json format stabilizes.

Manual quick check (Unix):
```
node scripts/verify-receipt-node.mjs artifacts/sample-receipt.json keys/ed25519_public.key
```
